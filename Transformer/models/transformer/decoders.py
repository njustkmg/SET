...
class MeshedDecoderLayer(Module):
    ...
    def forward(self, input, enc_output, mask_pad, mask_self_att, mask_enc_att):
        output = self.self_att(input, input, input, mask_self_att)
        self_att = output['out']
        ...
        output = self.enc_att(self_att, enc_output[:, 2], enc_output[:, 2], mask_enc_att)
        enc_att3 = output['out'] * mask_pad
